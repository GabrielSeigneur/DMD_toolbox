...
class DMDSetup:
    ...
    def __init__(
        ...
        Inputs:
        - theta_tilt: Tilt angle wrt to the normal to the DMD of the mirrors, in radians (default is 12 degrees).
        - tilt_direction: Tilt direction of the mirrors in the ON state, in radians (default is 135 degrees, i.e. towards the clipped corner of the DMD PCB).
        - mirror_pitch: Pitch of the mirrors in meters (default is 7.56 µm).
        - wavelength: Wavelength of the incident beam, in m (default is 638 nm).
        - path_file: Path to save the results (default is in the directory "Plots" that will be created in the root folder).
        """
        ...
        self.normal_mirror_ON = unitVecSphereToCartesianNormal(self.theta_tilt, self.tilt_direction)
        ...
        if custom_lat is not None:
            ...
            if not (-np.pi/2 <= self.lat_planar <= np.pi/2):
                raise ValueError("Custom latitude must be between -90° and 90° (in radians: -π/2 to π/2).")
            self.lat_planar = np.deg2rad(self.lat_planar)
        else:
            self.lat_planar = np.arctan(np.tan(self.theta_tilt)/np.sqrt(2)) # in rad - angle such that k_i and k_r are coplanar with the normal to the mirror in the ON state
    ...
    def get_phase_shift_XY(self, k_i):
        ...
        Inputs:
        - k_i: Incident wave vector, a 3D vector in Cartesian coordinates.
        Outputs:
        - delta_k_x: Phase shift in the x direction, normalised by the DMD spatial frequency (units of the DMD pitch).
        - delta_k_y: Phase shift in the y direction, normalised by the DMD spatial frequency (units of the DMD pitch).
        - mx: Closest inferior order in the x direction.
        - my: Closest inferior order in the y direction.
        """
    ...
    def get_all_phase_shifts(self):
        ...
        for i in range(len(self.latlong_array)): # iterate on the latitude_i
            for j in range(len(self.latlong_array)): # iterate on the longitude_i
                k_i = unitVecSphericalToCartesianEquator(self.latlong_array[i], self.latlong_array[j])
                delta_k_x, delta_k_y, mx, my = self.get_phase_shift_XY(k_i)
                self.phase_shifts[i, j] = np.sqrt(delta_k_x**2 + delta_k_y**2)
                self.orderX[i, j] = mx
                self.orderY[i, j] = my

        phaseShiftXYSingleLat = self.phase_shifts[self.idx_planar, :] # phase shift for a given latitude (the planar one)
        ...
        self.blazed_orders = [(self.orderX[self.idx_planar, self.blazed_longs_indices[i]], self.orderY[self.idx_planar, self.blazed_longs_indices[i]])
                              for i in range(len(self.blazed_longs_indices))]
        print(f"Found {len(self.blazed_orders)} blazed orders for lat = {np.rad2deg(self.lat_planar):.2f}°.")
    
    def plot_phase_shift(self):
        fig, ax = plt.subplots(1, 2, figsize=(20, 10))
        fig.suptitle(r'Phase Shifts for $\theta_{tilt}=$'+f" ${np.rad2deg(self.theta_tilt):.1f}"+'^{\circ}$'+ r' , $\lambda =$'+f" ${self.wavelength*10**9:.0f}$" \
                     +' $\mathrm{nm}$' + r' and $d_{mirror} = $'+ f"${self.mirror_pitch*1e6:.2f}$"+ ' $\mathrm{\mu m}$', fontsize='20', usetex = True)
        ...
        ax[0].axhline(y=np.rad2deg(self.lat_planar), color='darkolivegreen', linestyle='--', label = 'Tilt Direction')
        ax[0].text(self.theta_tilt-20, np.rad2deg(self.lat_planar)-2, r'$\mathbf{\mathrm{lat}_{i}=}$'+f' ${np.rad2deg(self.lat_planar):.2f}'+'^{\circ}$', color='white',\
                   fontsize=17, usetex = True) # type: ignore

        for i in range(len(self.blazed_longs_indices)):
            ax[0].axvline(x=np.rad2deg(self.latlong_array[self.blazed_longs_indices[i]]), color='maroon', linestyle='--', label = f'{self.blazed_orders[i]}')
            # print(np.rad2deg(latlong_array[blazed_longs_indices[i]]))

        ## Plot for one given latitude
        ax[1].plot(np.rad2deg(self.latlong_array), self.phase_shifts[self.idx_planar, :], color='slategray')
        ...
        ax[1].set_title(f'Phase Shift for ' + r'$\mathrm{lat}_{i}$ = ' + f'${np.rad2deg(self.lat_planar):.2f}'+'^{\circ}$', usetex = True, fontsize=14) # type: ignore
        ax[1].grid()
        ...
        plt.savefig(self.path_file + f'Phase_Shifts.png', dpi=300, bbox_inches='tight')

if __name__ == "__main__":
    dmd = DMDSetup()
    dmd.get_all_phase_shifts()
    dmd.plot_phase_shift()
    print(f"Blazed orders for latitude {np.rad2deg(dmd.lat_planar):.2f}°: {dmd.blazed_orders}")